...
// ETAP 2 ZALEWANIE B: settings-->
export class Et2b 
{ 
    visible:boolean = false;
    font_size:number = 0;
    
    CzasZalaniaB:SuperText;
    PrzerwaZalaniaB:SuperText;
    //CisnienieZalaniaB:SuperText;
    ...
    { 
       ...
       this.CzasZalaniaB = new SuperText("Czas zalania:",deviceWidth/8,deviceHeight/4,25,'rgb(44,214,255)','rgb(200,255,44)');
       this.CzasZalaniaB.value_type = "m/s ⏱️";
       this.CzasZalaniaB.Show();

       this.PrzerwaZalaniaB = new SuperText("Przerwa po zalaniu:",deviceWidth/8,this.CzasZalaniaB.y + this.CzasZalaniaB.height*2,25,'rgb(44,214,255)','rgb(200,255,44)');
       this.PrzerwaZalaniaB.value_type = "m/s ⏱️";
       this.PrzerwaZalaniaB.Show();

      /*  this.CisnienieZalaniaB = new SuperText("Ciśnienie zalania:",deviceWidth/8,this.PrzerwaZalaniaB.y + this.CzasZalaniaB.height*2,25,'rgb(44,214,255)','rgb(200,255,44)');
       this.CisnienieZalaniaB.value_type = " Kpa";
       this.CisnienieZalaniaB.Show(); */
    ...
    { 
      ...
      this.CzasZalaniaB.update(deviceWidth/13,deviceHeight/8);
      this.PrzerwaZalaniaB.update(deviceWidth/13,this.CzasZalaniaB.y + this.CzasZalaniaB.height*2);
      //this.CisnienieZalaniaB.update(deviceWidth/13,this.PrzerwaZalaniaB.y + this.CzasZalaniaB.height*2);
    ...
    { 
      ...
      ctx.lineJoin = 'round';
      ctx.lineWidth = 3;
      ctx.strokeText("2 ZALEWANIE B ⛲",deviceWidth/2,deviceHeight/35);
      ctx.lineWidth = 1;
      ctx.fillStyle = '#3faa0d';
      ctx.fillText("2 ZALEWANIE B ⛲",deviceWidth/2,deviceHeight/35);

      // super text:
      this.CzasZalaniaB.draw();
      this.PrzerwaZalaniaB.draw();
      //this.CisnienieZalaniaB.draw();
      ...
      var endwt_y = descriptionText("Czas zalania B:",text1,this.PrzerwaZalaniaB.x,this.PrzerwaZalaniaB.y + this.PrzerwaZalaniaB.height*1.8,deviceWidth/1.2,txt_height*1.3,'#0078FF','rgb(200,200,0)');
      var text2:string = "[0-60 min/sek] Wszystkie grzałki są wyłączone na wybrany czas, co spowoduje ustąpienie zalania.";
      endwt_y = descriptionText("Przerwa po zalaniu B:",text2,this.PrzerwaZalaniaB.x ,endwt_y + txt_height*2.2,deviceWidth/1.2, txt_height*1.3,'#0078FF','rgb(200,200,0)');
      ...
      if(options_screen.basic_set.CzujnikZalania_cb.Status() == true)
      {
        ...
        descriptionText("UWAGA CZUJNIK ZALANIA:",text3,this.PrzerwaZalaniaB.x ,endwt_y + txt_height*3,deviceWidth/1.2, txt_height*1.3,'#ffffFF','rgb(200,0,0)');
      }else
      {
    ...
    Onclick(x:number,y:number)
    {
      ...
      if(this.CzasZalaniaB.clicked(x,y))
      {
        play_key();
        DrawCircle(x,y,3,8,'#FFF200','red');
        this.ClickedOptionNumber = 1;
        this.addInput(x, y);
        this.input.title = "Czas zalania B";
        this.input.placeholder = "min/sek";
        return "";
      }

      if(this.PrzerwaZalaniaB.clicked(x,y))
      {
        play_key();
        DrawCircle(x,y,3,8,'#FFF200','red');
        this.ClickedOptionNumber = 2;
        this.addInput(x, y);
        this.input.title = "Przerwa zalania B";
        this.input.placeholder = "min/sek";
        return "";
      }

     /*  if(this.CisnienieZalaniaB.clicked(x,y))
      {
        play_button();
        DrawCircle(x,y,3,8,'#FFF200','red');
        this.ClickedOptionNumber = 3;
        this.addInput(x, y);
        this.input.title = "Ciśnienie zalania B";
    ...
    OnMouseMove(x:number,y:number)
    {
        ...
        if(this.CzasZalaniaB.hoover(x,y)      == true ||
           this.PrzerwaZalaniaB.hoover(x,y)   == true 
           //this.CisnienieZalaniaB.hoover(x,y) == true
           )
        {
      ...
      switch(this.ClickedOptionNumber)
      {
          case 1: // CzasZalaniaA
          {
            ...
            { 
               ...
               {
                this.CzasZalaniaB.value = (min*60)+sek;
                SendCommand("#CzasZalB:" + this.CzasZalaniaB.value); 
               }
            } 
          }
          break; 
          ...
          case 2: // PrzerwaZalaniaA
          {
            ...
            { 
               ...
               if(min <= 60 == true && sek <= 59 == true)
               {
                this.PrzerwaZalaniaB.value = (min*60)+sek;
                SendCommand("#PrzerwaZalB:" + this.PrzerwaZalaniaB.value);  
               }
            }  
          }
          break; 
         ...
         /*  case 3: // CisnienieZalaniaA
          {
               this.CisnienieZalaniaB.value = parseInt(this.input.value);
               if(this.CisnienieZalaniaB.value > 2000)
                this.CisnienieZalaniaB.value = 0;
          }
          break;  */
      }
